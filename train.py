...
nEpochs = 101  # Number of epochs for training 改动

resume_epoch = 0  # Default is 0, change if want to resume 从头开始训练
useTest = True # See evolution of the test set when training
nTestInterval = 20 # Run on test set every nTestInterval epochs
snapshot = 25 # Store a model every snapshot epochs 每过多少次保存一次模型
lr = 1e-5 # Learning rate

# myhands是自己的数据集
dataset = 'myhands' # Options: hmdb51 or ucf101

if dataset == 'hmdb51':
    num_classes=51
elif dataset == 'ucf101':
    num_classes = 101
elif dataset == 'myhands':
    num_classes = 2
else:
